...
func main() {
	drawingRepos, err := getDrawingRepos()
	if err != nil {
		panic(err)
	}

	s, err := newServer(drawingRepos)
	if err != nil {
		panic(err)
	}

	logger := getLogger()
	logger.Info().Interface("drawingRepos", drawingRepos).Int("port", s.config.port).Msg("starting server...")
